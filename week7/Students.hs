...
rawStudents :: [(String, String, Integer, Integer, Integer, Integer)]
...
examMark a = exam a
nonExamMark a = participation a + assignment1 a + assignment2 a
totalMark a = nonExamMark a + examMark a
pass a = percent (nonExamMark a) nonExamMax >= 40 && percent (examMark a) examMax >= 40 && percent (totalMark a) 100 >= 50

instance Ord Student where
  compare a b 
    | pass a < pass b = LT
    | pass a > pass b = GT
    | totalMark a < totalMark b = LT
    | totalMark a > totalMark b = GT
    | lastName a < lastName b = LT
    | lastName a > lastName b = GT
    | firstName a < firstName b = LT
    | firstName a > firstName b = GT
    | otherwise = EQ
